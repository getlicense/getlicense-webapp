...
folder are mapped to the application's root namespace, 'getlicense'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "getlicense"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'getlicense' in the same way
as `"getlicense/sass/src"`.
